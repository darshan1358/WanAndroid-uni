<template>
	...
	<view>
		<view class="content">
			<wuc-tab @change="tabChange" :tab-list="tabList" :tabCur.sync="TabCur" tab-class="text-center text-white bg-blue text-common" select-class="text-white"></wuc-tab>
...
export default {
	...
	onReachBottom() {
		...
		timer = setTimeout(function() {
			...
			this.getSwiperItemData(this.TabCur, this.pagesize);
	...
	methods: {
		...
		swiperChange(e) {
			...
			this.TabCur = current;
			this.getSwiperItemData(e.target.current, 0);
		},
		getSwiperItemData(itemIndex, pagesize) {
			...
			uni.request({
				...
				success(res) {
					// console.log("res.data.datas=="+JSON.stringify(res));
					that.swiperList = res.data.data.datas;
					// console.log('res.data.datas==' + JSON.stringify(res.data.data.datas));
				},
				complete() {
					if (pagesize == 1) {
