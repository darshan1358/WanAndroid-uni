...
export default {
	...
	onPullDownRefresh() {
		that.getSwiperItemData(this.TabCur, 1);
	},
	onLoad() {
		that = this;
		uni.request({
			...
			success(res) {
				...
				uni.request({
					url: 'https://www.wanandroid.com/project/list/1/json?cid=' + res.data.data[0].id,
					success(res) {
	...
	methods: {
		...
		getSwiperItemData(itemIndex, pagesize) {
			// console.log('itemIndex=' + itemIndex + '==' + JSON.stringify(this.tabList));
